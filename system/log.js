...
module.exports = function(type, data, color) {
    ...
    .replace(/%type/g, type || 'Cherry')
    .replace(/%hours/g, getTime('HH:mm:ss'))
    .replace(/%data/g, data));
}

module.exports.event = function({ event }) {
    var { type, threadID, messageID, senderID, author, userID } = event;
    return console.log(event);
}

module.exports.error = function(region, error) {
    return console.log(colors['error']
    .replace(/%type/g, region)
    .replace(/%hours/g, getTime('HH:mm:ss'))
    .replace(/%data/g, Array.isArray(error) || Object.isObject(error) ? JSON.stringify(error) : error.toString()));
}